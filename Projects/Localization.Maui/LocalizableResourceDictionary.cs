...
public class LocalizableResourceDictionary : ResourceDictionary, ISupportInitialize, ILocalizationTarget, ILocalizableResource
{
	...
	private void SaveTranslation(FileInfo locFileInfo)
	{
		try
		{
			...
			WriteResourceDictionaryAsXaml(fs, this);
		}
		catch (Exception ex)
		{
	...
	public void CreateTranslation(LocaleInfo locale)
	{
		...
		// save an empty resource dictionary
		WriteResourceDictionaryAsXaml(fs, Array.Empty<KeyValuePair<String, Object>>());
	}
	...
	private static void WriteResourceDictionaryAsXaml(Stream stream, IEnumerable<KeyValuePair<String, Object>> dictionaryEntries)
	{
		...
		writer.WriteStartElement("ResourceDictionary", "http://schemas.microsoft.com/dotnet/2021/maui");
		writer.WriteAttributeString("xmlns", "x", null, "http://schemas.microsoft.com/winfx/2009/xaml");

		foreach (KeyValuePair<String, Object> entry in dictionaryEntries)
		{
			writer.WriteStartElement("x", "String", "http://schemas.microsoft.com/winfx/2009/xaml");
			writer.WriteAttributeString("x", "Key", "http://schemas.microsoft.com/winfx/2009/xaml", entry.Key);
			writer.WriteString(entry.Value?.ToString() ?? String.Empty);
